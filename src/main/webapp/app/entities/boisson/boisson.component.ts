...
export class BoissonComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.boissonService
      ...
      .subscribe((res: HttpResponse<IBoisson[]>) => this.paginateBoissons(res.body, res.headers));
  }
  ...
  protected paginateBoissons(data: IBoisson[], headers: HttpHeaders) {
    ...
    data.forEach(item => {
      this.typeBoissonService
        ...
        .subscribe((res: ITypeBoisson) => (item.typeName = res.libelle), (res: HttpErrorResponse) => this.onError(res.message));
    });
  }
